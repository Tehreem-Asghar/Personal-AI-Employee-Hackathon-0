...
- [x] T002 [P] Initialize `.env` file with placeholders for Gmail and WhatsApp session paths (excluded from git).
- [x] T003 Initialize Playwright: `playwright install chromium`.
...
- [x] T004 Implement `credentials.py` in `src/utils/` to safely load environment variables.
- [x] T005 Implement `approval_handler.py` in `src/handlers/` to monitor the `AI_Employee_Vault/Approved` folder.
- [x] T006 Create `/Approved` and `/Plans` folders in the vault using `src/setup_vault.py`.
...
- [x] T007 [US1] Implement `gmail_watcher.py` in `src/watchers/` using Gmail API and OAuth2.
- [x] T008 [US1] Implement keyword filtering logic in `gmail_watcher.py` based on categorized list.
- [x] T009 [US1] Implement `whatsapp_watcher.py` in `src/watchers/` using Playwright with persistent context.
- [x] T010 [US1] Implement keyword filtering logic in `whatsapp_watcher.py`.
- [x] T011 [US1] Integrate `logger.py` to record every perception event in `/Logs`.
...
- [x] T012 [US2] Implement `email_server.py` in `src/mcp/` to expose `send_email` tool.
- [x] T013 [US2] Connect `approval_handler.py` to trigger `email_server.py` when an email draft is approved.
- [x] T014 [US2] Implement task movement from `/Approved` to `/Done` after successful send.
...
- [x] T015 [US3] Implement `planner.py` in `src/utils/` to format Claude's reasoning into markdown plans.
- [x] T016 [US3] Configure Claude Agent Skill to always call `planner.py` for tasks in `/Needs_Action`.
...
- [x] T017 [US4] Implement LinkedIn drafting logic in `src/utils/social_draft.py`.
- [x] T018 [US4] Implement `src/utils/linkedin_post.py` using Playwright for automated posting.
- [x] T019 [US4] Extend `approval_handler.py` to trigger `linkedin_post.py` when a LinkedIn draft is approved.
- [x] T020 [US4] Update `src/run_senses.ps1` to include the LinkedIn posting check.
...
- [x] T019 [P] Create a PowerShell script to bundle all watchers for Task Scheduler.
- [x] T020 Setup Windows Task Scheduler to run the system heartbeat every 15 minutes. (Instructions provided in README)
- [x] T021 Final verification of HITL safety: Ensure NO email sends without move to `/Approved`.
